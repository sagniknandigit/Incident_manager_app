...
export const createIncident = async (req: Request, res: Response) => {
    try {
        const { title, description, priority, imageUrl } = req.body;
...
export const getAllIncidents = async (req: Request, res: Response) => {
    try {
        const user = (req as any).user;
        const { status, priority, engineerId, page = 1, limit = 10 } = req.query;
        ...
        if (engineerId) whereClause.engineerId = Number(engineerId);
